...
def unsubscribe(request):
    ...
    if request.method == 'POST':
        addr = request.POST.get('email',False)
        if addr:
            try:
                e = EmailList.objects.get(email=addr)
                e.delete()
                p = Profile.objects.get(email=addr)
                p.email_list = False
                p.save()
            except:
                pass
        context['message'] = 'The email address %s has been removed from the list.' % addr
...
def officers(request):
    ...
    profiles = []
    try:
        profiles.append(Profile.objects.get(is_president=True))
    except:
        pass
    try:
        profiles.append(Profile.objects.get(is_vice_president=True))
    except:
        pass
    try:
        profiles.append(Profile.objects.get(is_secretary=True))
    except:
        pass
    try:
        profiles.append(Profile.objects.get(is_treasurer=True))
    except:
        pass
    if profiles:
...
def index(request):
    ...
    events = Events.objects.filter(date__gt=time_thresh).order_by('date')[:4]
    context = {
        'news' : news,
        'events' : events,
    }
...
def emailblast(request):
    ...
    if request.method == 'POST':
        try:
            ...
            for sub_list in [subscribers[i:i + 99] for i in xrange(0, len(subscribers), 99)]:
